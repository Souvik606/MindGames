...
const WordSearchGame = () => {
  ...
  const wordsToFind = ["CANVAS", "DROLL", "GREET", "HOE", "NET", "REPORT","SPIN","STUDY","TRAGEDY"];
  ...
  const isWordFound = wordCells.every((cell) =>
    newClickedCells.some(
      (clickedCell) => clickedCell.row === cell.row && clickedCell.col === cell.col
    )
  );
  ...
  return (
  <>
  {showInstructions?<WordSearchInstructions onProceed={()=>setShowInstructions(false)}/>:
  <div className="min-h-screen bg-gradient-to-br from-orange-200 via-orange-300 to-orange-400 flex items-center justify-center p-5">
      ...
      <div className="bg-white z-10 p-5 rounded-2xl shadow-lg max-w-6xl grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-5"> 
        <div className="flex-1 p-4 rounded-lg shadow-md bg-white border-4 border-orange-300">
          ...
          <h1 className="text-3xl font-bold text-orange-600 mb-4 text-center">Word Search</h1>
          <h1 className="text-3xl font-bold text-orange-600 mb-4 text-center">Timer: {formatTime(timer)}</h1>
          </div>
        ...
        <div className="flex-col justify-center items-center p-4 rounded-lg shadow-md bg-white border-4 border-orange-300">
          <h2 className="text-3xl font-bold text-orange-600 mb-4 text-center">Words to Find</h2>
          <ul className="flex lg:flex-col justify-center items-center flex-wrap gap-3">
            {wordsToFind.map((word, index) => (
              <li
                key={word}
                className={`text-lg font-semibold ${
                  foundWords.includes(index) ? "text-orange-800 line-through" : "text-orange-600"
                }`}
              >
                {word}
              </li>
      ...
      <CompletionModal isOpen={isModalOpen} time={timer} onPlayAgain={resetGame}/>
    </div>}
  </>
  )
};
